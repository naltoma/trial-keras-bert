...
def _get_vector(feature, aug_size):
    ...
    if aug_size == 1:
        ...
        tokens.extend(sp.encode(feature, out_type=str, enable_sampling=True, nbest_size=-1, alpha=0.01))
    tokens.append('[SEP]')
...
def _text_to_vectors(text, aug_size=1):
    print("_text_to_vectors(): begin.")
    vectors = []
    count = 0
    for sentences in text * aug_size:
        vectors.append(_get_vector(sentences, aug_size))
        count += 1
        if count % 1000 == 0:
...
text_train, text_test, y_train, y_test = train_test_split(text, labels, test_size=test_rate)
text_train, text_val, y_train, y_val = train_test_split(text_train, y_train, test_size=val_rate) # split train into train & val
#print("len(text_train), len(text_val), len(y_train): ", len(text_train), len(text_val), len(y_train))

y_train = np.tile(y_train, (aug_size, 1))

x_train = _text_to_vectors(text_train, aug_size)
x_val = _text_to_vectors(text_val)
x_test = _text_to_vectors(text_test)
print("len(x_train), len(y_train): ", len(x_train), len(y_train))
print("len(x_val), len(y_val): ", len(x_val), len(y_val))
print("len(x_test), len(y_test): ", len(x_val), len(y_val))
...
history = model.fit(x_train, y_train, epochs=EPOCHS, batch_size=BATCH_SIZE,
    callbacks = [
            ...
            ModelCheckpoint(filepath=model_path, monitor='val_loss', verbose=1, save_best_only=True, save_weights_only=False, mode='min', save_freq=1)
        ],
